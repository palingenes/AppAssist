...
object OldUtils {
    ...
    fun checkAccessibilityPermission(context: Activity, block: () -> Unit) {
        if (isServiceRunning(
                context,
                WorkAccessibilityService::class.java.canonicalName
                    ?: WorkAccessibilityService::class.java.name
            )
        ) {
            block()
        } else {
     ...
     * 关闭所有悬浮窗
     */
    fun closeAllSuspendWindow(windowManager: WindowManager, floatRootView: View) {

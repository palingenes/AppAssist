package com.wzy.assist.utils.net.utils
...
object NetUtils {
    ...
    fun getNetStatus(context: Context): @NetType String {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            val networkInfo = manager.getNetworkInfo(ConnectivityManager.TYPE_VPN)
            val isVpn = networkInfo?.isConnected ?: false
            if (isVpn) {
                return NetType.VPN
            }
    ...
    fun getNetStatus(ties: NetworkCapabilities): @NetType String {
        return if (!ties.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)) {
            ...
            if (ties.hasTransport(NetworkCapabilities.TRANSPORT_VPN)) {
                NetType.VPN
            } else if (ties.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) ||
